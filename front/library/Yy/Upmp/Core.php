...
function paraFilter($para) {
	...
	while ( list ( $key, $value ) = each ( $para ) ) {
		if ($key == Yy_Upmp_Config::SIGNATURE || $key == Yy_Upmp_Config::SIGN_METHOD || $value == "") {
			continue;
		} else {
			$result [$key] = $para [$key];
		}
...
function buildSignature($req) {
	...
	$prestr = $prestr.Yy_Upmp_Config::QSTRING_SPLIT.md5(Yy_Upmp_Config::$security_key);
	return md5($prestr);
}
...
function createLinkString($para, $sort, $encode) {
	...
	while (list ($key, $value) = each ($para)) {
		...
		$linkString.=$key.Yy_Upmp_Config::QSTRING_EQUAL.$value.Yy_Upmp_Config::QSTRING_SPLIT;
	}
	//去掉最后一个&字符
...
function post($url, $content = null)
{
	if (function_exists("curl_init")) {
		...
		// https verify
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, Yy_Upmp_Config::VERIFY_HTTPS_CERT);
		curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, Yy_Upmp_Config::VERIFY_HTTPS_CERT);
