...
if (function_exists("date_default_timezone_set")) {
	date_default_timezone_set(Yy_Upmp_Config::$timezone);
}

class Yy_Upmp_Service {
    ...
    static function trade($req, &$resp) {
    	...
    	$respString = post(Yy_Upmp_Config::$upmp_trade_url, $nvp);
    ...
    static function query($req, &$resp) {
    	$nvp = self::buildReq($req);
    	$respString = post(Yy_Upmp_Config::$upmp_query_url, $nvp);
    	return self::verifyResponse($respString, $resp);
    }
    ...
    static function buildReq($req) {
    	...
    	// 签名结果与签名方式加入请求
    	$filteredReq[Yy_Upmp_Config::SIGNATURE] = $signature;
    	$filteredReq[Yy_Upmp_Config::SIGN_METHOD] = Yy_Upmp_Config::$sign_method;
    ...
    static function verifySignature($para) {
    	$respSignature = $para[Yy_Upmp_Config::SIGNATURE];
    	// 除去数组中的空值和签名参数
